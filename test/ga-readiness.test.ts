...
function readReleaseTasks() {
  return readFileSync(join(repoRoot, "qualification/releases/1.1.1/tasks-v1.jsonl"), "utf8")
    .trim()
    .split("\n")
    .map((line) => JSON.parse(line));
}
...
describe("GA readiness contracts", () => {
  test("accepts the current legal 1.1 milestone lifecycle state", () => {
    ...
    expect(report.targetVersion).toBe("1.1.1");
  ...
  test("freezes every 1.1.1 public command with schemas and API operations", () => {
    const surface = readJson("qualification/releases/1.1.1/public-surface-v1.json");
    const commandIds = new Set(surface.commandManifest.commands.map((command: { id: string }) => command.id));

    expect(surface.frozenForVersion).toBe("1.1.1");
