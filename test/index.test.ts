  ...
  test("prints a machine-readable command manifest", async () => {
    ...
    expect(manifest.commands).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({
        ...
        options: expect.arrayContaining(["--from <date>", "--to <date>", "--user-id <id>", "--limit <n>", "--json", "--format <format>"]),
        capability: "read",
        apiEffect: "api-read",
        riskLevel: "medium",
        authRequired: true,
        jsonContract: expect.objectContaining({ successSchemaId: "admin-operations-response-v1" })
      }),
      expect.objectContaining({
        path: "search",
      ...
      expect.objectContaining({
        ...
        examples: expect.arrayContaining([
          expect.objectContaining({ command: 'apexcn draft question --title "标题" --problem "问题描述" --research-file ./research.json --save --json', mode: "execute" })
        ])
      }),
      expect.objectContaining({
        path: "draft reply",
        options: expect.arrayContaining(["--topic-id <id>", "--answer <text>", "--topic-file <path>", "--research-file <path>", "--save", "--format <format>"]),
        safety: expect.objectContaining({ effects: ["read", "config-write"], preview: "none" }),
        examples: expect.arrayContaining([
          expect.objectContaining({ command: 'apexcn draft reply --topic-id 30549 --answer "回复建议" --save --json', mode: "execute" })
        ])
      }),
      expect.objectContaining({
        path: "review topic",
  ...
  test("command manifest includes stable agent guidance for every leaf command", async () => {
    ...
    expect(manifest.commands).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({
        ...
        examples: expect.arrayContaining([
          expect.objectContaining({ command: "apexcn auth set-token --profile agent-prod --base-url https://oracleapex.cn/ords/api --token-env APEXCN_API_KEY", mode: "execute" })
        ])
      })
    ]));
  });
