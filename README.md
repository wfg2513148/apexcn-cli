...
也可以对 AI 说：“请执行 `apexcn update`，把 apexcn-cli 升级到官方最新版，并告诉我结果。”不需要重新输入 API Key；升级会保留认证配置、备份旧版本，失败时恢复旧安装。此命令用于官方安装器管理的安装。如果旧版本提示没有 `update` 命令，先按上面的安装步骤重新安装一次，之后即可使用。
...
遇到值得复用的讨论时，可以让 AI 先收藏；如果后续回复也值得关注，再订阅该话题。这样下一次排查同类问题时，可以先在个人内容中检索，而不是从整个社区重新开始。
