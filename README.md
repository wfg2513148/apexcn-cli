...
> 请在本机安装 apexcn-cli。只使用官方安装器；安装后运行版本检查并告诉我结果。安装阶段不要向我索取、记录或显示 API Key。

安装器会安装 CLI，并显示升级、回滚和卸载方法。安装程序需要 Node.js 20 或更高版本；缺少时，让 AI 先安装合适的 Node.js 再继续。

### 2. 在 AI 工具中安全配置 API Key

先在 [APEX 中文社区](https://oracleapex.cn/) 登录账号，在 **API Key 管理** 中复制 Key。不要把 Key 粘贴到普通聊天、帖子、截图或 issue 中。

在 AI 工具提供的安全密钥或环境变量设置中，把 Key 保存为 `APEXCN_API_KEY`。保存后，向 AI 发送：

> 我已在本机的安全环境变量中设置 `APEXCN_API_KEY`。请配置 apexcn-cli 使用该变量，检查本地认证和社区连接，告诉我当前登录账号与检查结果。不要输出或回显 API Key。

AI 会在后台执行认证和诊断，不需要你打开命令行。若所用 AI 工具没有安全密钥或环境变量设置，请更换为支持该能力的本地 AI 工具；不要把 API Key 发到普通对话中。
...
阅读与搜索由 AI 在后台调用 `apexcn-cli` 完成。发布、修改、删除、收藏、订阅和标记答案时，AI 仍会先显示预览，只有你明确确认后才会执行。
