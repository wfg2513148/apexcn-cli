...
- 收藏话题或回复、订阅帖子，并标记有权限管理的正确答案；
- 打开个人看板，并只在自己创建、回复、收藏或订阅的内容中搜索。
...
安装完成后，安装器会显示 `Installed source`，并紧接着给出当前安装可直接复制的升级、回滚和卸载命令。它们由安装目录中的 `scripts/lifecycle-agent.*` 执行，不是 `apexcn` 子命令；自定义安装目录也无需再次填写路径。升级会先备份，若远端最新版本比当前版本更旧则拒绝降级并自动恢复。

例如 macOS / Linux 默认安装会显示类似命令：

```bash
bash "$HOME/.apexcn/tools/apexcn-cli/package/scripts/lifecycle-agent.sh" upgrade
bash "$HOME/.apexcn/tools/apexcn-cli/package/scripts/lifecycle-agent.sh" rollback --backup "<升级输出的备份路径>" --yes
bash "$HOME/.apexcn/tools/apexcn-cli/package/scripts/lifecycle-agent.sh" uninstall --yes
```

请优先复制安装器实际输出的命令，因为默认目录与自定义目录可能不同。卸载会保留认证配置。
...
当你让本地 AI 根据社区资料回答 APEX 问题时，CLI 会向 AI 返回帖子、正确答案、社区链接、原文链接和证据编号，由本地 AI 组织答案。这个检索过程只读取社区搜索与帖子详情，不会改变网页中的现有知识问答功能。
...
以下内容可以直接发给 AI。涉及发布、修改或删除时，建议保留“先预览、等我确认”的要求。帖子和回复编号应从搜索结果或账号内容中选择，不要把示例编号当作当前线上内容。
...
5. **总结指定帖子**
   ...
   > 请从刚才的搜索结果中选择一个帖子，总结主要内容、操作步骤、注意事项和真实链接。
...
17. **起草友好回复**
    ...
    > 请为刚才选中的帖子起草一条友好的回复，说明我的解决方法并补充必要步骤。先预览，不要发布。

18. **回复已有回复**

   > 请从刚才选中的帖子中选择一条回复，补充我的测试结果。先确认它属于这个帖子，再给我预览，等我确认后发布。

19. **删除本人回复**

   > 请从我的回复列表中选择一条回复，确认它属于当前账号并允许删除。先显示内容和真实链接，等我确认后按安全流程删除。
