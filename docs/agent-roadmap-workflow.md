# Roadmap, release and independent validation

Load this policy only for a user-requested milestone, formal release, or CLI
capability extension. Paths in the preserved protocol are repository-relative
unless absolute. The current user request bounds continuation and external work;
this document does not turn ordinary edits into an automatic release.

## Roadmap Execution Protocol

- For an explicitly requested roadmap milestone or formal release, read `roadmap.json` and active `issues.json`; ordinary local edits do not activate this protocol.
...
- After that summary, release verification, and context compaction, automatically mark the predecessor completion review `approved` and activate the next planned milestone only within the user-authorized milestone set. Do not wait for additional user confirmation inside that set. Do not infer permission to start an unrelated milestone, create a task, or publish from an ordinary local edit.
...
- Resolve the current independent CLI validation project from `roadmap.json.testingBindings.validator.project`, then run `node scripts/check-roadmap.mjs --validator-readiness` before starting a round. The former `/Users/kwang/Downloads/Works/66.Projects/apexcn-cli-test` is a historical location, not an executable fallback. See `validation-routing.md`.
...
- Resolve a live ORDS API task from the registered server repository and current task list. `roadmap.json.testingBindings.server` stores historical IDs; verify cwd/status before reusing a task.
...
- After the CLI change passes focused and full local quality gates, freeze an immutable candidate artifact and checksum. Then create a fresh user-visible Codex Desktop validation task whose session `cwd` is exactly the resolved independent validation project.
