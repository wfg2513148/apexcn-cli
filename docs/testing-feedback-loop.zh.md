...
每个里程碑都必须启动一个独立 Codex 目标模式。该目标模式必须持续到里程碑验收条件 100% 通过、独立验证完成、阻塞问题清零、提交推送、发版和上下文交接全部完成，不得提前结束。自 `0.4` 里程碑起，每个里程碑必须新建一个 Codex Desktop 中用户可见的主实施任务，任务 `cwd` 必须精确等于主仓库目录；不得在上一里程碑的任务中继续执行下一里程碑。上一里程碑完成发布验证和上下文压缩后，主会话自动创建并交接给新的里程碑任务，无需再次等待用户确认。
...
- validator 必须是 Codex Desktop 中用户可见的任务，任务元数据 `cwd` 必须精确等于 `/Users/kwang/Downloads/Works/66.Projects/apexcn-cli-test`，确保它显示在侧边栏的 `apexcn-cli-test` 项目下；隐藏 subagent 不能替代 validator 线程。
...
发布验证和上下文压缩完成后，自动批准上一里程碑完成审查，并把下一个计划里程碑改为 `in_progress`；同一时间仍只允许一个活动里程碑。
