...
| `0.8` | `0.80.x` | 组织治理与生命周期资格化 | `in_progress` | 0 | `approved` | `not_due` |
...
| ID | Capability | Status | User value |
|---|---|---|---|
| `M060-CAP-CRUD` | 主题与回复 CRUD | `validated` | 用户可通过稳定命令管理主题和回复。 |
| `M060-CAP-REVIEW` | 草稿与审核门禁 | `validated` | 正文质量、隐私和密钥问题在 API preview 前被发现。 |
| `M060-CAP-WORKFLOW` | hash-bound workflow | `validated` | preview、approval 和 execute 绑定同一请求并可恢复审计。 |
| `M060-CAP-TEST-ENV` | 隔离写测试环境 | `validated` | 真实写流程可验证且不污染生产社区或遗留测试资源。 |
| `M060-CAP-VISUAL` | 回写页面视觉验收 | `validated` | 用户在真实页面看到的内容、格式、状态和权限与写请求一致。 |
| `M060-CAP-AGENT-PREVIEW` | Agent preview-only 写工具 | `validated` | Agent 可生成受策略约束的写预览，但不能执行写操作。 |

### 核心验收

| ID | Status | Metric | Target | Measurement |
|---|---|---|---|---|
| `M060-AC-001` | `pass` | 至少 20 个完整写 workflow 首次尝试样本通过。 | `>= 20 workflows` | isolated dev@oci validator run |
| `M060-AC-002` | `pass` | 成功执行的 preview、approval 和 execute 请求 hash 100% 一致。 | `= 100 percent` | workflow artifact hash comparison |
| `M060-AC-003` | `pass` | 缺确认、过期审批、正文变化和权限不足全部阻断。 | `= 100 percent` | negative workflow matrix |
| `M060-AC-004` | `pass` | delete 确认绕过为零。 | `= 0 bypasses` | delete preview approval and execute attack scenarios |
| `M060-AC-005` | `pass` | resume 和 retry 不产生重复写入。 | `= 0 writes` | idempotence and resume scenarios |
| `M060-AC-006` | `pass` | 401/409/429/timeout 恢复不产生不确定重复写。 | `= 0 writes` | fault-injected isolated workflow runs |
| `M060-AC-007` | `pass` | workflow、审核和测试证据中无密钥泄露。 | `= 0 findings` | artifact and log secret scan |
| `M060-AC-008` | `pass` | 隔离测试资源残留为零。 | `= 0 resources` | post-run isolated environment inventory |
| `M060-AC-009` | `pass` | 当前里程碑不存在活动 P0/P1 问题。 | `= 0 issues` | issues.json entries scoped to milestone 0.6 |
| `M060-AC-010` | `pass` | 20 个隔离写 workflow 全部具备同一对象的后端证据，以及系统 Google Chrome 在 apexcn forums 既有 APEX 页面上的视觉证据。 | `= 100 percent` | dual evidence validator report against an existing apexcn forums APEX page |
| `M060-AC-011` | `pass` | 全部 preview-only MCP 工具返回 willExecute=false 且真实写请求为零。 | `= 0 bypasses` | preview tool call and network trace |
