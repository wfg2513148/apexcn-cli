...
本手册面向在终端中直接使用 apexcn-cli 的开发者。除安装和认证章节外，示例均假设已经完成安装与账号配置。示例中的帖子和回复编号只用于说明命令格式；执行前请通过搜索、列表或当前账号内容取得有效编号。
...
每个公开 JSON 命令都有独立、版本化的成功响应 Schema。可用 `apexcn schema list --json` 发现，`apexcn schema show <schema-id> --json` 查看，或用 `apexcn schema bundle --output schemas.json --json` 导出完整契约集。全部命令共享 `apexcn-error-v1` 错误 Schema；Schema ID 或版本发生变化时，调用方应重新生成或验证解析器。
...
`apexcn auth set-token <token>` 把 key 保存到权限为 `0600` 的默认 `prod` profile，不回显 key，也不调用社区 API。兼容旧版的 `-apikey <token>` 仍可使用。引号只用于保护 shell 特殊字符；命令行参数可能进入 shell 历史和短暂出现在进程列表中。`--token-env <name>` 只保存环境变量名，适合更高安全要求。同时传 `--token-env` 和 token 参数或 `--token` 时，运行时优先使用环境凭据，缺失或无效时回退到文件凭据；两个 backend 都没有可用 token 时，API 命令会在发起请求前 fail closed。token 必须由可用于 HTTP header 的可见 ASCII 字符组成，不能包含空白，也不能是 `你的_API_KEY`、`YOUR_API_KEY` 等示例占位符。`--base-url` 必须是绝对 `http` 或 `https` URL。
查看当前 profile：
...
如果 API 返回 401 且提示 token 被拒绝，请在社区复制或重新生成 Key 后运行：
...
第一条命令更新默认 `prod` 配置；第二条命令会检查账号、板块和搜索 API。浏览器登录状态不会替代 CLI 的 API Key。
...
`collection favorites` 遍历服务端认证只读 cursor，只将话题收藏写入离线合集，并保留完整正文、URL、topicId、收藏时间、更新时间和 provenance。回复收藏会以 `REPLY_FAVORITE_EXCLUDED` 明确报告并保留 topicId/replyId 供诊断，不会把 replyId 当成 topicId。import 要求目标目录为空；restore 只覆盖 bundle 管理的文件，不删除无关文件。
...
为本地 AI 检索可引用的社区证据，不在 CLI 或服务端生成答案：
...
使用 App 100 既有的服务端社区知识问答。该命令为兼容路径，与 `rag retrieve` 相互独立：
