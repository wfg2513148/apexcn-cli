...
async function approve({ id, runDir, config }) {
  const result = await candidate(cliArgs([
    ...
    "--approved-by", "m060-r31-validator",
    ...
    const requests = [
      {
        ...
        params: {
          ...
          capabilities: {},
          clientInfo: { name: "m060-r31-validator", version: "1" }
        }
      },
...
async function runLive() {
  ...
  try {
    for (const [groupId, operations] of LIVE_GROUPS) {
      const current = {
        title: `M060 R31 ${groupId}`,
        updatedTitle: `M060 R31 ${groupId} updated`,
        topicId: null,
        topicVersion: null,
        replyId: null,
        replyVersion: null,
