...
function parseArgs(argv) {
  ...
  const roundRootArgument = values.get("round-root");
  if (!roundRootArgument) {
    throw new Error("--round-root is required");
  }
  const roundRoot = resolve(roundRootArgument);
  return {
    mode,
    roundRoot,
...
async function approve({ id, runDir, config }) {
  const result = await candidate(cliArgs([
    "workflow", "approve", "--run-dir", runDir,
      "--approved-by", "m060-r32-validator",
    ...
    const requests = [
      {
        ...
        params: {
          ...
          capabilities: {},
      clientInfo: { name: "m060-r32-validator", version: "1" }
        }
      },
...
async function browserEvidence(browser, {
  ...
  try {
    ...
    if (status === expectedStatus && requiredText.length > 0) {
...
function existingApexTopicUrlPass(value, topicId) {
  if (typeof value !== "string" || value.includes("/api/v1/") || value.includes("/visual")) {
    return false;
  }
  try {
    const parsed = new URL(value);
    return parsed.protocol === "https:"
      && parsed.hostname === "oracleapex.cn"
      && parsed.pathname === "/ords/f"
      && parsed.searchParams.get("p") === `102:14:::::P14_THREAD_ID:${topicId}`
      && Boolean(parsed.searchParams.get("cs"));
  } catch {
    return false;
  }
}
...
async function runLiveLifecycle({
  ...
  if (operation === "topic-delete") {
    ...
    invariant(existingApexTopicUrlPass(current.visualUrl, current.topicId), `${reference} existing APEX topic URL invalid`);
  }
  ...
  if (operation === "topic-create") {
    ...
    invariant(existingApexTopicUrlPass(current.visualUrl, current.topicId), `${reference} create existing APEX topic URL invalid`);
    ledger.push({
      groupId,
      type: "topic",
      id: result.id,
  ...
  if (operation === "topic-delete") {
    ...
    visual = await browserEvidence(browser, {
      ...
      expectedStatus: 200,
      requiredText: ["Thread data unavailable"],
...
async function cleanupActiveResources({ ledger, pacer, browser, files }) {
  ...
  for (const entry of [...ledger].reverse()) {
    ...
    invariant(existingApexTopicUrlPass(current.visualUrl, entry.id), `${reference} existing APEX topic URL invalid`);
    const runDir = join(ROOT, "cleanup-runs", reference);
...
async function runLive() {
  ...
  try {
    for (const [groupId, operations] of LIVE_GROUPS) {
      const current = {
        title: `M060 R32 ${groupId}`,
        updatedTitle: `M060 R32 ${groupId} updated`,
        topicId: null,
        topicVersion: null,
        replyId: null,
        replyVersion: null,
...
async function browserLoopbackSelfCheck(root) {
  const notFoundHtml = "<!doctype html><html><head><title>M060 synthetic 404</title></head><body>synthetic not found</body></html>";
  const delayedHtml = "<!doctype html><html><head><title>M060 delayed visual</title></head><body>topic ready<script>setTimeout(() => { document.body.insertAdjacentText('beforeend', ' reply rendered'); }, 250);</script></body></html>";
  ...
  const server = createServer((request, response) => {
    ...
    if (request.method === "GET" && request.url === "/delayed") {
      delayedRequests += 1;
      response.writeHead(200, {
        "Content-Type": "text/html; charset=utf-8",
        "Content-Length": Buffer.byteLength(delayedHtml),
        Connection: "close"
      });
      response.end(delayedHtml);
      return;
    }
    response.writeHead(404, {
      "Content-Type": "text/html; charset=utf-8",
  ...
  try {
    ...
    const delayedResponse = await page.goto(`http://127.0.0.1:${address.port}/delayed`, {
      waitUntil: "domcontentloaded",
      timeout: 10_000
    });
    await page.waitForFunction(
      (texts) => texts.every((text) => document.body?.innerText.includes(text)),
      ["topic ready", "reply rendered"],
      { timeout: 2_000 }
    );
    const delayedBody = await page.locator("body").innerText();
    invariant(delayedResponse.status() === 200, "delayed browser self-check status");
    invariant(delayedBody.includes("reply rendered"), "delayed browser self-check wait");
    await page.close();
  } finally {
    await context?.close();
    await new Promise((resolvePromise) => server.close(resolvePromise));
  ...
  invariant(notFoundRequests === 1, `browser self-check 404 requests ${notFoundRequests}`);
  invariant(delayedRequests === 1, `browser self-check delayed requests ${delayedRequests}`);
  return {
    pass: true,
    status: 404,
    title: "M060 synthetic 404",
    documentRequests: notFoundRequests + delayedRequests,
    delayedRenderWait: true
  };
}
