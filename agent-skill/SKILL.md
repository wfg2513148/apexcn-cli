...
- For natural Oracle APEX troubleshooting/how-to questions, use `apexcn rag retrieve "<question>" --top-k 5 --json` even when the user does not explicitly say "APEX 中文社区". The command retrieves community evidence; the local AI owns synthesis. Bind material claims to `evidenceId` values internally, but make each user-visible citation a Markdown link whose label is the evidence's full `title` and whose target is `communityUrl`; never show a bare `S1`, `S4`, or other evidence id as the visible label.
- When `apexcn ask` is explicitly requested, apply the same visible-citation rule to its `sources` and `provenance.sources`: use each full `title` as the link label, `communityUrl` as the target, and keep `evidenceId` only as the machine-verifiable identifier.
...
- Use `apexcn auth audit --json` and `apexcn me capabilities --require-capability admin-operations --json` before `apexcn admin operations` when an administrator asks about apexcn-cli usage, failures, or search keyword trends. Apply `--from` and `--to` together for a custom inclusive window of at most 90 days, plus exact `--user-id` and bounded `--limit` as requested. Treat results as cooperative server-received request attribution, not unforgeable process telemetry; never claim that local validation or pre-server network failures are included, and never infer ask question text from keyword aggregates.
...
- For `rag retrieve`, preserve `answerability`, every cited `evidenceId`, full `title`, `communityUrl`, optional `originalUrl`, and `provenance.requestIds`. If `answerability.status` is `unanswerable`, do not manufacture an answer; if it is `partial`, state the evidence limitation.
